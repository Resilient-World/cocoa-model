...
DEFAULT_CASEJ_CHECKPOINT = Path(__file__).resolve().parents[2] / "models" / "casej_surrogate.pt"
...
def load_casej_surrogate(
    ...
    *,
    ...
    if path.is_file():
        ...
        logger.info("Loaded CASEJSurrogate from %s", path)
    else:
        logger.warning("CASEJ checkpoint missing at %s; using uninitialized weights", path)
    model.eval()
    return model
