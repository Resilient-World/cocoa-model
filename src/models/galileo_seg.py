...
DEM) are encoded via :class:`~models.galileo_backbone.GalileoCocoaBackbone` and
...
from models.galileo_backbone import GalileoCocoaBackbone
...
# Galileo encoder time bands (see :mod:`models.vendor.galileo_data_utils`)
...
def load_galileo_seg_checkpoint(
    ...
    *,
    ...
    model.to(device)
    model.eval()
    logger.info("Loaded GalileoCocoaSegmentation from %s", path)
    return model
