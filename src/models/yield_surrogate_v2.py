...
class YieldSurrogateV2(YieldSurrogateModel):
    ...
    def from_v1_checkpoint(
        ...
        *,
        ...
        if other_missing:
            logger.warning("Checkpoint missing non-PAPE keys: %s", other_missing[:8])
        if unexpected:
            logger.warning("Unexpected checkpoint keys: %s", unexpected[:8])
        if pape_missing:
            logger.info("PAPE keys absent (%d); using zero-init identity", len(pape_missing))
