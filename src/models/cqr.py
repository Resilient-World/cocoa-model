...
class QuantileYieldSurrogate(nn.Module):
    """
    Same mechanistic + GRU/MLP backbone as :class:`~models.yield_surrogate.YieldSurrogateModel`,
...
class ConformalCalibrator:
    ...
    def save(self, path: str | Path) -> None:
        ...
        logger.info("Saved CQR calibrator to %s", path)
...
def load_quantile_yield_model(
    ...
    *,
    ...
    if path.is_file():
        ...
        logger.info("Loaded QuantileYieldSurrogate from %s", path)
    else:
        logger.warning("CQR checkpoint missing at %s; using random weights", path)
    model.to(device)
    model.eval()
    return model
